...
metadata {
    definition (name: "NUE ZigBee Wall Switch Double Gang 1.3", namespace: "Hubitat", author: "George Castanza") {
        capability "Actuator"
...
//	log.debug cluster
    ...
    if (descMap.cluster == "0006" && descMap.attrId == "0000" && descMap.value =="01" && descMap.endpoint == "${epid1}") {
		...
		else if (descMap.cluster == "0006" && descMap.attrId == "0000" && descMap.value =="01" && descMap.endpoint == "${epid2}") {
...
def off1() {
	...
	sendEvent(name: "switch1", value: "off")
   	"st cmd 0x${device.deviceNetworkId} 0x${epid1} 0x0006 0x0 {}" 
  }

def on1() {
   log.info "on1()"
	sendEvent(name: "switch1", value: "on")
    "st cmd 0x${device.deviceNetworkId} 0x${epid1} 0x0006 0x1 {}" 
    }

def off2() {
    log.info "off2()"
	sendEvent(name: "switch2", value: "off")
   	"st cmd 0x${device.deviceNetworkId} 0x${epid2} 0x0006 0x0 {}" 
  }

def on2() {
   log.info "on2()"
	sendEvent(name: "switch2", value: "on")
    "st cmd 0x${device.deviceNetworkId} 0x${epid2} 0x0006 0x1 {}" 
    }
...
def refresh() {
	log.info "refreshing"
    [
        "st rattr 0x${device.deviceNetworkId} 0x${epid1} 0x0006 0x0", "delay 1000",
